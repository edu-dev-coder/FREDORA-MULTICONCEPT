...
import { db, reportsTable, testSessionsTable, profilesTable } from "@workspace/db";
import {
...
router.get("/reports", async (req, res): Promise<void> => {
  ...
  const reports = await db
    .select()
    .from(reportsTable)
    .where(params.data.userId ? eq(reportsTable.userId, params.data.userId) : undefined)
...
router.post("/reports/generate/:sessionId", async (req, res): Promise<void> => {
  ...
  const [session] = await db
    ...
    .where(eq(testSessionsTable.id, params.data.sessionId as `${string}-${string}-${string}-${string}-${string}`));
  ...
  // Payment gate removed — reports available for all completed sessions
