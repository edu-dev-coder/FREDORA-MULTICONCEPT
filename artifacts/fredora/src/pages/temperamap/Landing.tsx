...
const AVATAR_COLORS = ["#1B3A6B", "#C8961E", "#1E8449", "#C0392B", "#2E4053", "#6C3483", "#148F77", "#B03A2E"];
...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [tRes, fRes, faqRes] = await Promise.all([
        ...
        if (!cancelled && tRes.ok) {
          ...
          if (list.length) {
            setTestimonials(
              list.map((t, i) => ({
                name: t.authorName || "Verified User",
                role: t.company || "Verified Customer",
                avatar: (t.authorName || "V").charAt(0).toUpperCase(),
        ...
        if (!cancelled && fRes.ok) {
          ...
          if (list.length) {
            setFeatures(
              list.map((feat) => ({
                icon: FEATURE_ICONS[String(feat.icon || "").toLowerCase()] ?? "✨",
                title: feat.title,
        ...
        if (!cancelled && faqRes.ok) {
          const data = await faqRes.json();
          const list = Array.isArray(data) ? data : [];
          if (list.length) {
            setFaqs(list.map((faq) => ({ q: faq.question, a: faq.answer })));
          }
        }
      } catch {
        // fall back to the hardcoded defaults on any error
      }
    };
    load();
    return () => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-border shadow-sm">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="sm" onClick={() => setLocation("/")} className="text-muted-foreground hover:text-foreground text-xs gap-1.5 rounded-full">
              ← Fredora Group
            </Button>
          ...
          <div className="flex items-center gap-3">
            <Button variant="ghost" onClick={() => setLocation("/temperamap/sign-in")} className="text-primary font-medium">
              Sign In
            </Button>
            <Button onClick={() => setLocation("/temperamap/sign-up")} className="bg-accent hover:bg-accent/90 text-white font-semibold rounded-full shadow-sm">
              Get Started
            </Button>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden bg-linear-to-br from-primary via-[#1e4580] to-[#0f2549] text-white py-24 px-4">
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <Badge className="mb-6 bg-accent/20 text-accent border-accent/30 text-xs sm:text-sm px-4 py-1">
          ...
          <p className="text-base sm:text-xl text-blue-100 mb-8 sm:mb-10 max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center max-w-sm sm:max-w-none mx-auto">
            <Button
              size="lg"
              onClick={() => setLocation("/temperamap/sign-up")}
              className="w-full sm:w-auto bg-accent hover:bg-amber-600 text-white font-bold text-base sm:text-lg px-6 sm:px-8 py-5 sm:py-6 shadow-lg rounded-full"
            >
              Take the Test
            </Button>
            <Button
              size="lg"
              variant="outline"
              onClick={() => setLocation("/temperamap/sign-in")}
              className="w-full sm:w-auto border-white/30 text-white hover:bg-white/10 text-base sm:text-lg px-6 sm:px-8 py-5 sm:py-6 rounded-full"
            >
              Sign In
            </Button>
          </div>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 justify-center mt-8 sm:mt-12">
            {TEMPERAMENTS.map(t => (
              <div key={t.name} className="flex items-center gap-1.5 sm:gap-2 bg-white/10 rounded-full px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm backdrop-blur">
                <span>{t.emoji}</span>
      ...
      {/* Stats bar */}
      <section className="bg-accent py-6 sm:py-8 px-4">
        <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 text-center">
          {[
            ...
            <div key={stat.label}>
              ...
              <div className="text-amber-100 text-xs sm:text-sm font-medium mt-0.5 sm:mt-1">{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-accent/10 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <Button
            size="lg"
            onClick={() => setLocation("/temperamap/sign-up")}
            className="bg-primary hover:bg-primary/90 text-white font-bold text-lg px-10 py-6 rounded-full shadow-lg"
          >
      ...
      <footer className="bg-primary text-blue-200 py-10 px-4 text-center text-sm">
        ...
        <div className="flex flex-wrap justify-center gap-4 text-xs text-blue-300/80 mb-3">
          <a href="/temperamap/terms" className="hover:text-accent transition-colors">Terms of Service</a>
          <span>·</span>
          <a href="/temperamap/privacy" className="hover:text-accent transition-colors">Privacy Policy</a>
          <span>·</span>
