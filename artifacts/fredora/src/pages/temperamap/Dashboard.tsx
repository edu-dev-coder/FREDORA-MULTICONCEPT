  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button
              ...
              size="sm"
              onClick={() => setLocation("/")}
              className="text-white/80 hover:text-white hover:bg-white/10 text-xs rounded-full"
            >
              ← Fredora Group
            </Button>
          ...
          <div className="flex items-center gap-3">
            ...
            {user?.role === "admin" && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => setLocation("/temperamap/admin/dashboard")}
                className="text-xs font-bold border-accent/40 text-accent hover:bg-accent hover:text-white rounded-full transition-colors"
              >
                Admin Panel ↗
              </Button>
            )}
            <Button
              variant="ghost"
              size="sm"
              onClick={() => { logout(); setLocation("/temperamap"); }}
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
            ...
            <Card className="border-2 border-accent/20 bg-linear-to-br from-accent/5 to-primary/5">
              <CardContent className="py-14 px-8 text-center">
                ...
                <Button
                  ...
                  className="bg-accent hover:bg-amber-600 text-white font-bold px-10 py-6 text-base shadow-lg rounded-full"
                >
        ...
        {hasTests && (
          <Card className="mb-8 bg-linear-to-r from-primary to-[#1e4580] text-white border-0 shadow-md">
            <CardContent className="p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <Button
                onClick={() => setLocation("/temperamap/select-test")}
                className="bg-accent hover:bg-amber-600 text-white font-bold shrink-0 rounded-full"
              >
                Start Assessment →
              </Button>
            </CardContent>
                ...
                <Card
                  ...
                  onClick={() => {
                    if (test.status === "completed") {
                      setLocation(`/temperamap/results/${test.id}?type=${test.testType}`);
                    } else {
                      setLocation(`/temperamap/test/${test.id}`);
                    }
                  }}
                >
      ...
      <footer className="mt-12 py-8 px-4 text-center border-t border-border">
        ...
        <div className="flex flex-wrap justify-center gap-4 text-xs text-muted-foreground/60">
          ...
          <span>·</span>
          <a href="/temperamap/admin/dashboard" className="hover:text-accent transition-colors">TemperaMap Admin</a>
        </div>
      </footer>
    </div>
  );
