...
import { db, tmTestSessionsTable } from "@workspace/db";
import {
...
router.get("/tests", async (req, res): Promise<void> => {
  ...
  const sessions = await db
    ...
    .where(params.data.userId ? eq(tmTestSessionsTable.userId, params.data.userId) : undefined)
    .orderBy(desc(tmTestSessionsTable.createdAt));
...
router.get("/tests/:id", async (req, res): Promise<void> => {
  ...
  const [session] = await db
    .select()
    .from(tmTestSessionsTable)
    .where(eq(tmTestSessionsTable.id, params.data.id));
  if (!session) {
