...
import { computeWorkplaceProfile } from "@/lib/workplace";
...
export default function TakeTest() {
  ...
  const saveAnswersToServer = useCallback(async (ans: Record<string, number>) => {
    if (!sessionId || Object.keys(ans).length === 0) return;
    setSaving(true);
    try {
  ...
  // Debounced auto-save: saves 1.5s after last answer change
  useEffect(() => {
  ...
  }, [answers, saveAnswersToServer]);

  // Save immediately when tab is hidden (phone call, app switch)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (session?.status === "completed") {
      setLocation(`/temperamap/results/${sessionId}`);
      return;
    }
  ...
  const coreQuestions = questions.filter(q => q.temperament !== "bonus" && q.temperament !== "response");
  const bonusQuestions = questions.filter(q => q.temperament === "bonus");
  const responseQuestions = questions.filter(q => q.temperament === "response");
  ...
  const currentQuestions: Question[] = isBonusPage
    ? [...bonusQuestions, ...responseQuestions]
  ...
  const submitTest = useMutation({
    mutationFn: async () => {
      ...
      const workplace = !isChildTest && session?.testType === "corporate_team"
        ? computeWorkplaceProfile(answers, questions, {
    ...
    onSuccess: () => {
      ...
      if (session?.testType === "couples_test") {
        setLocation(`/temperamap/invite-partner/${sessionId}`);
      } else {
        setLocation(`/temperamap/results/${sessionId}`);
      }
    },
        ...
        <div className="max-w-2xl mx-auto">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-blue-200 text-sm">
              {saving && <span className="text-yellow-300 mr-1">saving...</span>}
            ...
            <CardContent className="p-4">
              ...
              <p className="text-xs text-muted-foreground">
                {session?.testType === "couples_test"
                  ...
                  : session?.testType === "corporate_team"
                  ? "These 14 questions complete your workplace temperament and leadership profile."
            ...
            return (
              ...
              >
                <CardContent className="p-4 sm:p-5">
                  <p className="font-medium text-primary mb-4 text-sm sm:text-base leading-relaxed">
                  ...
                  <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
                    {(isChildTest ? CHILD_OPTIONS : OPTIONS).map(opt => (
                      <button
                        ...
                        className={`min-h-[44px] py-2.5 px-3 rounded-lg text-xs font-semibold border-2 transition-all flex items-center justify-center text-center ${
                          selected === opt.value
                            ? "bg-primary border-primary text-white shadow-sm"
                            : "border-border hover:border-primary/50 hover:bg-primary/5 text-muted-foreground active:scale-[0.98]"
                        }`}
                      >
                        {opt.label}
                      </button>
